...
module Election_result = struct
            ...
            (function
             | `List xs ->
                ...
                |> List.map
                     (function
                      | `Int i -> i
                      | _ -> failwith "int expected in Election_result.wrap_nonhomomorphic"
                     )
                |> Array.of_list
             | _ -> failwith "(inner) list expected in Election_result.wrap_nonhomomorphic"
            )
       |> Array.of_list
    | _ -> failwith "(outer) list expected in Election_result.wrap_nonhomomorphic"
  ...
  let wrap s x =
    ...
    let rec loop : 'a . 'a Question_signature.t -> Yojson.Safe.t list -> 'a t =
      ...
      let r : a t =
        ...
        | NonHomomorphic (_, s), x :: xs -> NonHomomorphic (wrap_nonhomomorphic x, loop s xs)
  ...
  let nth x i =
    if i < 0 then (
      invalid_arg "Election_result.nth: negative index"
    ) else (
  ...
  let map2 (type c) f xs ys =
    ...
    let rec loop : 'a . int -> 'a t -> c list -> c list =
      ...
      match xs with
      | Nil ->
         if i = n then List.rev accu else fail ()
      | Homomorphic (x, xs) ->
         if i < n then (
...
module Atd_shape_j = struct
  ...
  let write_shape = write_shape

  let read_shape = read_shape
end

module Atd_shape_bs = Atd_shape_j
